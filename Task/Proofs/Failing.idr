...
failingMeansNoInteraction (Edit (Named n) (Select ts)) @{EditIsNormal}         s is_f =
  let is_all = hoist_all {p=failing . snd} {l=ts} is_f
      is_empty = all_true_empties_filter is_all
   in rewrite is_empty in (Refl, Refl)
...
failingMeansNoInteraction (Step t1 c)                  @{StepIsNormal n1}      s is_f with (failingMeansNoInteraction t1 s is_f)
  failingMeansNoInteraction (Step t1 c)                @{StepIsNormal n1}      s is_f | (is_v1, is_i1) = (Refl, rewrite is_i1 in rewrite is_v1 in Refl)
