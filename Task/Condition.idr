...
data Condition : Task h a -> Type where
  Failing   : {t : Task h a} -> {auto n : IsNormal t} -> {s : Heap h} -> (IsTrue  (failing t), IsNothing (value t s), IsNil  (inputs t s)) -> Condition t
  Finished  : {t : Task h a} -> {auto n : IsNormal t} -> {s : Heap h} -> (IsFalse (failing t), IsJust    (value t s), IsNil  (inputs t s)) -> Condition t
  Recurring : {t : Task h a} -> {auto n : IsNormal t} -> {s : Heap h} -> (IsFalse (failing t), IsJust    (value t s), IsCons (inputs t s)) -> Condition t
  Running   : {t : Task h a} -> {auto n : IsNormal t} -> {s : Heap h} -> (IsFalse (failing t), IsNothing (value t s), IsCons (inputs t s)) -> Condition t
  Stuck     : {t : Task h a} -> {auto n : IsNormal t} -> {s : Heap h} -> (IsFalse (failing t), IsNothing (value t s), IsNil  (inputs t s)) -> Condition t
