...
data Condition : Task h a -> Type where
  Failing   : {t : Task h a} -> {s : State h} -> (IsTrue  (failing t), IsNothing (value t s), IsNil  (inputs t s)) -> Condition t
  Finished  : {t : Task h a} -> {s : State h} -> (IsFalse (failing t), IsJust    (value t s), IsNil  (inputs t s)) -> Condition t
  Recurring : {t : Task h a} -> {s : State h} -> (IsFalse (failing t), IsJust    (value t s), IsCons (inputs t s)) -> Condition t
  Running   : {t : Task h a} -> {s : State h} -> (IsFalse (failing t), IsNothing (value t s), IsCons (inputs t s)) -> Condition t
  Stuck     : {t : Task h a} -> {s : State h} -> (IsFalse (failing t), IsNothing (value t s), IsNil  (inputs t s)) -> Condition t

||| Covering function for the `Condition` view.
condition : (t : Task h a) -> (s : State h) -> Condition t
condition t s with (isItTrue (failing t))
  condition t s | Yes yes_f = let (nope_v, nope_i) = interaction t s yes_f in Failing (yes_f, nope_v, nope_i)
